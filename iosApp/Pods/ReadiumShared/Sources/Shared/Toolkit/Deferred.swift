...
public final class Deferred<Success, Failure: Error> {
    ...
    ///
    /// Can be useful to return early a value in a `.flatMap` or `deferred { ... }` construct.
    public class var cancelled: Deferred<Success, Failure> {
        Deferred {
            $0(.cancelled)
        }
    }
    ...
    public func resolve(on completionQueue: DispatchQueue? = .main, _ completion: @escaping Completion = { _ in
        ...
        if let queue = queue {
            queue.async {
                self.closure(completionOnQueue)
            }
        } else {
    ...
    /// its result.
    public func also(on queue: DispatchQueue? = nil, _ block: @escaping (Success) -> Void) -> Deferred<Success, Failure> {
        map(
            on: queue,
            success: { val, compl in
                block(val)
    ...
    public func delay(for seconds: TimeInterval, on queue: DispatchQueue? = nil) -> Deferred<Success, Failure> {
        asyncMap(on: queue) { output, completion in
    ...
    private func map<NewSuccess, NewFailure>(
        ...
        cancelled: @escaping (@escaping NewCompletion<NewSuccess, NewFailure>) -> Void = {
            $0(.cancelled)
        }
