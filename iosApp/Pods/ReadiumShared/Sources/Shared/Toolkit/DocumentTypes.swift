...
public struct DocumentTypes {
    ...
    init(infoDictionary: [String: Any]?) {
        ...
        .flatMap {
            $0.tags(withClass: .mediaType)
        }
        .compactMap {
            MediaType($0)
        }

        let utisFileExtensions = utis.flatMap {
            $0.tags(withClass: .fileExtension)
        }
...
public struct DocumentType: Equatable, Loggable {
    ...
    init?(dictionary: [String: Any]) {
        ...
        let mediaTypes = mediaTypeStrings.compactMap {
            MediaType($0)
        }
        .removingDuplicates()
