...
public struct Link: JSONEquatable, Hashable, Sendable {
    ...
    public init(
        ...
        self.init(
            href: href,
            mediaType: (jsonObject["type"] as? String).flatMap {
                MediaType($0)
            },
            templated: templated,
...
public extension Array where Element == Link {
    ...
    init(
        ...
        let links = json.compactMap {
            try? Link(json: $0, warnings: warnings)
        }
    ...
    func firstIndex<T: Equatable>(withProperty otherProperty: String, matching: T, recursively: Bool = false) -> Int? {
        firstIndex {
            ($0.properties.otherProperties[otherProperty] as? T) == matching
        }
    }
