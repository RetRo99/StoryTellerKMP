...
public class HTMLResourceContentIterator: ContentIterator {
    ...
    private func parseElements() async throws -> ParsedElements {
        ...
        .eraseToAnyError()
        .tryMap {
            try SwiftSoup.parse($0)
        }
        .tryMap {
            try parse(document: $0, locator: locator, beforeMaxLength: beforeMaxLength)
        }
        .asyncMap {
            await adjustProgressions(of: $0)
        }
        resource.close()
    ...
    private class ContentParser: NodeVisitor {
        ...
        func tail(_ node: Node, _ depth: Int) throws {
            if let node = node as? TextNode {
                ...
                let language = try node.language().map {
                ...
                segmentsAcc.append(TextContentElement.Segment(
                    locator: baseLocator.copy(
                        ...
                        text: { [self] in
                            $0 = Locator.Text.trimming(
                                text: rawTextAcc,
                                before: (wholeRawTextAcc?.suffix(beforeMaxLength)).map {
                                    String($0)
                                }
                            )
                        }
...
private extension Locator.Text {
    static func trimming(text: String, before: String?) -> Locator.Text {
        let leadingWhitespaceIdx = text.firstIndex {
            !$0.isWhitespace && !$0.isNewline
        } ?? text.startIndex
        let leadingWhitespace = String(text[..<leadingWhitespaceIdx])

        let trailingWhitespaceIdx = text.lastIndex {
            !$0.isWhitespace && !$0.isNewline
        }
        .map {
            text.index(after: $0)
