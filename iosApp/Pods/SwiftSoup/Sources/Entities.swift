...
public class Entities {
    ...
    public class EscapeMode: Equatable {
        ...
        fileprivate lazy var entitiesByCodepoint = entitiesByName.sorted() { a, b in
            a.scalar < b.scalar
        }
        ...
        private static let codeDelims = Set([",", ";"].flatMap {
            $0.utf8
        })
        ...
        public func codepointForName(_ name: String) -> UnicodeScalar? {
            let ix = entitiesByName.binarySearch {
                $0.name < name
            }
            guard ix < entitiesByName.endIndex else {
                return nil
            }
            let entity = entitiesByName[ix]
            guard entity.name == name else {
                return nil
            }
        ...
        public func nameForCodepoint(_ codepoint: UnicodeScalar) -> String? {
            var ix = entitiesByCodepoint.binarySearch {
                $0.scalar < codepoint
            }
            var matches: [String] = []
