...
public final class CharacterReader {
    ...
    public func unconsume() {
        guard pos > input.startIndex else {
            return
        }
    ...
    public func matches(_ seq: String, ignoreCase: Bool = false, consume: Bool = false) -> Bool {
        ...
        for scalar in scalars {
            ...
            if ignoreCase {
                guard c.uppercase == scalar.uppercase else {
                    return false
                }
            } else {
                guard c == scalar else {
    ...
    public func nextIndexOf(_ seq: String) -> String.UTF8View.Index? {
        ...
        while true {
            guard let firstCharIx = input[start...].firstIndex(of: targetUtf8.first!) else {
                return nil
            }
    ...
    static let dataTerminators = Set([.Ampersand, .LessThan, TokeniserStateVars.nullScalr].flatMap {
        $0.utf8
    })
    ...
    static let tagNameTerminators = Set([.BackslashT, .BackslashN, .BackslashR, .BackslashF, .Space, .Slash, .GreaterThan, TokeniserStateVars.nullScalr].flatMap {
        $0.utf8
    })
