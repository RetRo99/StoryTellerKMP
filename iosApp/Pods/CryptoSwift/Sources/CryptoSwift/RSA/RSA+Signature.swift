...
extension RSA: Signature {
    ...
    public func verify(signature: Array<UInt8>, for bytes: Array<UInt8>, variant: SignatureVariant) throws -> Bool {
        ...
        guard signature.count == self.keySizeBytes else {
        ...
        if expectedData.count == self.keySizeBytes && expectedData.prefix(1) == [0x00] {
            expectedData = Array(expectedData.dropFirst())
        }
        ...
        guard signatureResult == expectedData else {
            return false
        }
    ...
    fileprivate static func hashedAndEncoded(_ bytes: [UInt8], variant: SignatureVariant, keySizeInBytes: Int) throws -> Array<UInt8> {
        ...
        guard variant.enforceLength(hashedMessage, keySizeInBytes: keySizeInBytes) else {
            throw RSA.Error.invalidMessageLengthForSigning
        }
        ...
        guard padded.count == keySizeInBytes else {
            throw RSA.Error.invalidMessageLengthForSigning
        }
