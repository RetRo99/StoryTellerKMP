    ...
    public init?<S: StringProtocol>(_ text: S, radix: Int = 10) {
        ...
        while start != text.startIndex {
            ...
            if count == charsPerWord {
                guard let d = Word.init(text[start..<end], radix: radix) else {
                    return nil
                }
                words.append(d)
                end = start
...
extension String {
    ...
    public init(_ v: CS.BigUInt) {
        self.init(v, radix: 10, uppercase: false)
    }
    ...
    public init(_ v: CS.BigUInt, radix: Int, uppercase: Bool = false) {
        ...
        if power == 0 {
            parts = v.words.map {
                String($0, radix: radix, uppercase: uppercase)
            }
        } else {
            parts = []
            var rest = v
