...
public final class Scrypt {
    ...
    public init(password: Array<UInt8>, salt: Array<UInt8>, dkLen: Int, N: Int, r: Int, p: Int) throws {
        ...
        guard !(N < 2 || (N & (N - 1)) != 0) else {
            throw Error.nMustBeAPowerOf2GreaterThan1
        }

        guard N <= .max / 128 / r else {
            throw Error.nIsTooLarge
        }
        guard r <= .max / 128 / p else {
            throw Error.rIsTooLarge
        }
