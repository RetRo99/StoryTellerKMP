...
extension Entry.LocalFileHeader {
    var validFields: [Field] {
        var fields: [Field] = []
        if self.uncompressedSize == .max {
            fields.append(.uncompressedSize)
        }
        if self.compressedSize == .max {
            fields.append(.compressedSize)
        }
        return fields
    }
}
...
extension Entry.CentralDirectoryStructure {
    var validFields: [Field] {
        ...
        if self.relativeOffsetOfLocalHeader == .max {
            fields.append(.relativeOffsetOfLocalHeader)
        }
        if self.diskNumberStart == .max {
            fields.append(.diskNumberStart)
        }
        return fields
    }
    var zip64ExtendedInformation: Entry.ZIP64ExtendedInformation? {
        self.extraFields?.compactMap {
            $0 as? Entry.ZIP64ExtendedInformation
        }
        .first
    }
}
...
extension Entry.ZIP64ExtendedInformation {
    ...
    init?(data: Data, fields: [Field]) {
        let headerLength = 4
        guard fields.reduce(0, { $0 + $1.size }) + headerLength == data.count else {
            return nil
        }
    ...
    init?(zip64ExtendedInformation: Entry.ZIP64ExtendedInformation?, offset: UInt64) {
        ...
        guard let existingInfo = zip64ExtendedInformation else {
            return nil
        }
