...
extension Archive.ZIP64EndOfCentralDirectoryRecord {
    ...
    init?(data: Data, additionalDataProvider provider: (Int) throws -> Data) {
        guard data.count == Archive.ZIP64EndOfCentralDirectoryRecord.size else {
            return nil
        }
        guard data.scanValue(start: 0) == zip64EOCDRecordSignature else {
            return nil
        }
        ...
        guard self.versionNeededToExtract >= Archive.Version.v45.rawValue else {
            return nil
        }
...
extension Archive.ZIP64EndOfCentralDirectoryLocator {
    ...
    init?(data: Data, additionalDataProvider provider: (Int) throws -> Data) {
        guard data.count == Archive.ZIP64EndOfCentralDirectoryLocator.size else {
            return nil
        }
        guard data.scanValue(start: 0) == zip64EOCDLocatorSignature else {
            return nil
        }
