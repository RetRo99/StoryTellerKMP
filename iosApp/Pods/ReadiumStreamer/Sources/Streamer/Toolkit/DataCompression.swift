...
public extension Data {
    ...
    func gunzip() -> Data? {
        ...
        guard ftr.isize == UInt32(truncatingIfNeeded: inflated.count) else {
            return nil
        }
        guard ftr.crc32 == inflated.crc32().checksum else {
            return nil
        }
        return inflated
    }
...
public struct Crc32: CustomStringConvertible {
    ...
    private static func loadCrc32fromZLib() -> ZLibCrc32FuncPtr? {
        guard let libz = dlopen("/usr/lib/libz.dylib", RTLD_NOW) else {
            return nil
        }
        guard let fptr = dlsym(libz, "crc32") else {
            return nil
        }
...
public struct Adler32: CustomStringConvertible {
    ...
    private static func loadAdler32fromZLib() -> ZLibAdler32FuncPtr? {
        ...
        guard let fptr = dlsym(libz, "adler32") else {
            return nil
        }
