...
final class EPUBMetadataParser: Loggable {
    ...
    private func titleElements(ofType titleType: EPUBTitleType) -> [ReadiumFuzi.XMLElement] {
        ...
        .compactMap { meta in
            ...
            return metas["title", in: .dcterms].first {
                $0.id == id
            }?
            .element
        }
        ...
        .sorted { title1, title2 in
            let order1 = title1.attr("id").flatMap {
                displaySeqs[$0]
            } ?? ""
            let order2 = title2.attr("id").flatMap {
                displaySeqs[$0]
            } ?? ""
    ...
    private func accessibilityProfiles() -> [Accessibility.Profile] {
        ...
        return hrefs.compactMap {
            accessibilityProfile(from: $0)
        }
    }
    ...
    private func accessibilityAccessModes() -> [Accessibility.AccessMode] {
        metas["accessMode", in: .schema]
            .map {
                Accessibility.AccessMode($0.content)
            }
    }
    ...
    private func accessibilityFeatures() -> [Accessibility.Feature] {
        metas["accessibilityFeature", in: .schema]
            .map {
                Accessibility.Feature($0.content)
            }
    }
    ...
    private func accessibilityHazards() -> [Accessibility.Hazard] {
        metas["accessibilityHazard", in: .schema]
            .map {
                Accessibility.Hazard($0.content)
            }
    }
    ...
    private func parseContributors() -> (
        ...
        for element in findContributorElements() {
            ...
            let roles = element.attr("id")
                .map { id in
                    metas["role", refining: id].map(\.content)
                }
                ?? []
    ...
    private func collection(from meta: OPFMeta) -> Metadata.Collection? {
        ...
        return Metadata.Collection(
            name: name,
            identifier: meta.id.flatMap {
                metas["identifier", in: .dcterms, refining: $0].first?.content
            },
            sortAs: meta.id.flatMap {
                metas["file-as", refining: $0].first?.content
            },
            position: meta.id
            .flatMap {
                metas["group-position", refining: $0].first?.content
            }
            .flatMap {
                Double($0)
            }
