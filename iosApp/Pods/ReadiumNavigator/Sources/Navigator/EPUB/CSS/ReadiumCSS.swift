...
extension ReadiumCSS {
    mutating func update(with settings: EPUBSettings) {
        ...
        userProperties = CSSUserProperties(
            ...
            textColor: settings.textColor.map {
                CSSIntColor($0.rawValue)
            },
            backgroundColor: settings.backgroundColor.map {
                CSSIntColor($0.rawValue)
            },
            ...
            lineHeight: settings.lineHeight.map {
                .unitless($0)
            },
            paraSpacing: settings.paragraphSpacing.map {
                CSSRemLength($0)
            },
            paraIndent: settings.paragraphIndent.map {
                CSSRemLength($0)
            },
            wordSpacing: settings.wordSpacing.map {
                CSSRemLength($0)
            },
            letterSpacing: settings.letterSpacing.map {
                CSSRemLength($0 / 2)
            },
            bodyHyphens: settings.hyphens.map {
                $0 ? .auto : .none
            },
            ligatures: settings.ligatures.map {
                $0 ? .common : .none
            },
            ...
            overrides: [
                "font-weight": settings.fontWeight
                    .map {
                        String(format: "%.0f", (Double(CSSStandardFontWeight.normal.rawValue) * $0).clamped(to: 1...1000))
                    }
                    ?? "",
            ]
        )
...
private extension Element {
    ...
    func lang() -> Language? {
        let code = (try? attr("xml:lang")).takeIf {
            !$0.isEmpty
        }
            ?? (try? attr("lang")).takeIf {
            !$0.isEmpty
        }
