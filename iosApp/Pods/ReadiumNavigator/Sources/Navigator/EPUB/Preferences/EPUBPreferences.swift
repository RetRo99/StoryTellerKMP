...
public struct EPUBPreferences: ConfigurablePreferences {
    ...
    public init(
        ...
        self.letterSpacing = letterSpacing.map {
            max($0, 0)
        }
        self.ligatures = ligatures
        self.lineHeight = lineHeight
        self.pageMargins = pageMargins.map {
            max($0, 0)
        }
        self.paragraphIndent = paragraphIndent
        self.paragraphSpacing = paragraphSpacing.map {
            max($0, 0)
        }
        ...
        self.theme = theme
        self.typeScale = typeScale.map {
            max($0, 0)
        }
        self.verticalText = verticalText
        self.wordSpacing = wordSpacing.map {
            max($0, 0)
        }
    }
